...
file_path = (base_path / "datasets" / "const_2vox" / "2const_exc_data.csv").resolve()
file_path2 = (base_path / "datasets" / "const_2vox" / "norm_2const_exc.csv").resolve()

test_data = pd.read_csv(file_path)
t_span = np.linspace(0, test_data.iloc[1]['Time'], 150)
#M_ic = [test_data.iloc[0]['Mx_IC'], test_data.iloc[0]['My_IC'],test_data.iloc[0]['Mz_IC']] this is for one voxel
M_ic_1 = [test_data.iloc[1]['Mx_IC_1'], test_data.iloc[1]['My_IC_1'], test_data.iloc[1]['Mz_IC_1']]
...
predictions = model.predict(test_features_np[1:2])
predictions_pd = pd.DataFrame(predictions, columns=[['B_x', 'B_y']])
#print(test_features_np[1:2])
#print(test_features_np)

M_true = solve_bloch(t_span, M_ic_1, test_data.iloc[1]['B_x'], test_data.iloc[1]['B_y'], test_data.iloc[1]['B_z'])
M_predicted = solve_bloch(t_span, M_ic_1, predictions_pd.iloc[0]['B_x'], predictions_pd.iloc[0]['B_y'], test_data.iloc[1]['B_z'])
